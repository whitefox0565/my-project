$(document).ready(function () {
	...
	$(document).on('click', '.mapViewGymEach', function () {
		...
		$.ajax({
			...
			success: function (result) {
				...
				$("#mapGymLink").text(selectedGymName + " 선택");
	...
	// 특정 요소가 보이도록 설정
	$(document).on('click', '.mapViewGymEach', function () {
		$('#mapViewGymName, .mapViewGymContents, #mapViewLink #mapGymLink').css("display", "flex");  // Show the gym info box
	});
	...
	// mapGymLink 클릭 시 이벤트 핸들러 추가
	$('#mapGymLink').on('click', function() {
	    var selectedGymName = $('#mapGymLink').text().replace(" 선택", ""); // 선택된 체육관 이름
	    
		// 부모 창의 특정 요소에 값 전달
		if (window.opener && !window.opener.closed) {
	         // 부모 창의 `updatePayGymName` 함수 호출
       		 window.opener.updatePayGymName(selectedGymName);
	    }
	    // 현재 창 닫기
	    window.close();
	});
